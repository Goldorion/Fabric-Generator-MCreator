...
<#assign ge = block.getGeneratableElement()>
    if(${block}.hasBE) ${block}BE = Registry.register(Registry.BLOCK_ENTITY_TYPE,new Identifier("${modid}","${block}be"),BlockEntityType.Builder.create(${block}.${block}BE::new).build(null));
