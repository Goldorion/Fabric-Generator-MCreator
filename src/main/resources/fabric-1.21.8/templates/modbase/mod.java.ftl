...
public class ${JavaModName} implements ModInitializer {
	...
	@Override public void onInitialize() {
		...
		<#if w.getGElementsOfType('dimension')?filter(e -> hasProcedure(e.onPlayerLeavesDimension) || hasProcedure(e.onPlayerEntersDimension) || e.enablePortal)?size != 0>${JavaModName}Dimensions.load();</#if>		<#if types["guis"]??>${JavaModName}Menus.load();</#if>
