<#-- @formatter:off -->
<#include "procedures.java.ftl">
<#if field$condition?has_content>
	<#assign conditions = generator.procedureNamesToObjects(field$condition)>
<#else>
	<#assign conditions = ["", ""]>
</#if>
this.goalSelector.add(${customBlockIndex+1}, new Goal() {
	{
		this.setControls(EnumSet.of(Goal.Control.MOVE));
	}

	public boolean canStart() {
		if (${name}Entity.this.getTarget() != null && !${name}Entity.this.getMoveControl().isMoving()) {
			<#if hasProcedure(conditions[0])>
			double x = ${name}Entity.this.getPosX();
			double y = ${name}Entity.this.getPosY();
			double z = ${name}Entity.this.getPosZ();
			Entity entity = ${name}Entity.this;
			</#if>
			return <#if hasProcedure(conditions[0])><@procedureOBJToConditionCode conditions[0]/><#else>true</#if>;
		} else {
			return false;
		}
	}

	@Override public boolean shouldContinue() {
		<#if hasProcedure(conditions[1])>
		double x = ${name}Entity.this.getPosX();
		double y = ${name}Entity.this.getPosY();
		double z = ${name}Entity.this.getPosZ();
		Entity entity = ${name}Entity.this;
		</#if>
		return <#if hasProcedure(conditions[1])><@procedureOBJToConditionCode conditions[1]/> &&</#if>
			${name}Entity.this.getMoveControl().isMoving() && ${name}Entity.this.getTarget() != null && ${name}Entity.this.getTarget().isAlive();
	}

	@Override public void start() {
		LivingEntity livingentity = ${name}Entity.this.getTarget();
		Vec3d vec3d = livingentity.method_30950(1);
		${name}Entity.this.moveControl.moveTo(vec3d.x, vec3d.y, vec3d.z, ${field$speed});
	}

	@Override public void tick() {
		LivingEntity livingentity = ${name}Entity.this.getTarget();
		if (${name}Entity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
			${name}Entity.this.tryAttack(livingentity);
		} else {
			double d0 = ${name}Entity.this.squaredDistanceTo(livingentity);
			if (d0 < ${field$radius}) {
				Vec3d vec3d = livingentity.method_30950(1);
				${name}Entity.this.moveControl.moveTo(vec3d.x, vec3d.y, vec3d.z, ${field$speed});
			}
		}
	}
});
<#-- @formatter:on -->