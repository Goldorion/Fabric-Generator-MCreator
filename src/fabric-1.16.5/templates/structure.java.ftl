...
public class ${name}Structure {
    public static void init() {
        Feature<DefaultFeatureConfig> feature = Registry.register(Registry.FEATURE, new Identifier("${modid}", "${registryname}"), new Feature<DefaultFeatureConfig>(DefaultFeatureConfig.CODEC) {
            @Override
            public boolean generate(StructureWorldAccess worldAccess, ChunkGenerator chunkGenerator, Random random, BlockPos pos, DefaultFeatureConfig config) {
                ...
                if ((random.nextInt(1000000) + 1) <= ${data.spawnProbability}) {
                	...
                	for(int a = 0; a < count; a++) {
                		...
                		int j = world.getTopY(Heightmap.Type.<#if data.surfaceDetectionType=="First block">WORLD_SURFACE_WG<#elseif data.surfaceDetectionType=="First motion blocking block">OCEAN_FLOOR_WG</#if>, i, k);
        ...
        RegistryKey<ConfiguredFeature<?, ?>> configFeatKey = RegistryKey.of(Registry.CONFIGURED_FEATURE_WORLDGEN,
                ...
                Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, configFeatKey.getValue(), feature.configure(DefaultFeatureConfig.DEFAULT));
